...
class BrowserPage(QWebEnginePage):
    ...
    def certificateError(self, error):
        print("[CERT]", error.errorDescription())
        return True
...
class MainWindow(QMainWindow):
    ...
    def crypto_mode_changed(self, mode):

        self.settings["crypto_mode"] = mode
        self.save_settings()

        self.status_bar.showMessage(
           f"Encryption Mode Selected: {mode}",
           5000
        )

        print(f"[CRYPTO MODE] {mode}")
        ...
        with open(
           os.path.join(runtime_dir, "mode.json"),
           "w"
        ) as f:
    ...
    def get_crypto_mode(self):
        return self.crypto_selector.currentText()
    
    def check_tls_security(self, url):

        session = self.load_pqc_session()

        if session is None:
            self.security_indicator.setText("⚠ No Session")
            return

        status = session["status"]

        if status == "Hybrid PQC":
            self.security_indicator.setText("🔐 PQC")

        elif status == "Classical TLS":
            self.security_indicator.setText("🔒 TLS")

        else:
            self.security_indicator.setText("⚠ Unknown")

        self.status_bar.showMessage(

            f'{session["key_exchange"]} | '
            f'{session["cipher_suite"]}',
    ...
    def __init__(self):
        ...
        self.PQC_SESSION_FILE = "proxy_logs/current_session.json"
        # --- Navigation Bar ---
        ...
        self.timer.timeout.connect(self.refresh_security)
        ...
        self.url_bar.returnPressed.connect(
            self.navigate_to_url_or_search
        )
        ...
        self.crypto_selector.setToolTip(
            "Select Encryption Mode"
        )

        self.crypto_selector.addItems([
            "Classical TLS",
            "PQC Hybrid"
        ])

        saved_mode = self.settings.get(
            "crypto_mode",
            "Classical TLS"
        )

        index = self.crypto_selector.findText(saved_mode)

        if index >= 0:
            self.crypto_selector.setCurrentIndex(index)

        self.crypto_selector.currentTextChanged.connect(
            self.crypto_mode_changed
        )

        navbar.addWidget(self.crypto_selector)
        # HTTPS Lock Icon
        self.security_indicator = QAction("⚠ Unknown", self)
        navbar.addAction(self.security_indicator)
        ...
        self.status_bar.showMessage(
            f"Current Mode: {self.get_crypto_mode()}",
            5000
        )
        
        self.security_indicator.setText("🔐 PQC Protected")
        navbar.addAction(self.security_indicator)
        
        report_btn = QAction("📄 Security Report", self)
        report_btn.triggered.connect(self.show_security_report)
        navbar.addAction(report_btn)
    ...
    def load_pqc_session(self):

        if not os.path.exists(self.PQC_SESSION_FILE):
            return None

        try:
            with open(self.PQC_SESSION_FILE, "r") as f:
                return json.load(f)
        ...
        if self.tabs.currentWidget() and self.tabs.currentWidget().browser == browser:
            ...
            if url.scheme() == "https":
                ...
                if not lock_pixmap.isNull():
                    self.security_indicator.setIcon(QIcon(lock_pixmap.scaled(16, 16, Qt.KeepAspectRatio, Qt.SmoothTransformation)))
                    self.security_indicator.setToolTip("Secure (HTTPS)")
                else:
                    # Fallback if built-in icon is not found
                    self.security_indicator.setIcon(QIcon()) # Clear icon
                    self.security_indicator.setToolTip("Secure (HTTPS) - Icon not loaded. Resource missing.")
                    print("Warning: Built-in lock icon not found. Using empty icon.")
            else:
                self.security_indicator.setIcon(QIcon()) # Clear icon for non-HTTPS
                self.security_indicator.setToolTip("Insecure or No Connection")
    ...
    def load_finished(self, success, browser):
        ...
        if success:
            ...
            print("Load Failed :", browser.url().toString())
    ...
    def update_ui_for_current_tab(self):
        ...
        if browser:
            ...
            if hasattr(self, "security_indicator"):
                self.security_indicator.setText("⚠ Unknown")# Clear icon
    ...
    def refresh_security(self):
        ...
        if browser:
            ...
            self.check_tls_security(browser.url())
    ...
    def show_security_report(self):
        ...
        if session is None:
            QMessageBox.warning(
                self,
                "No Session",
                "No TLS session information found."
            )
            return
    ...
    Handshake Time:
    {session["handshake_time_ms"]} ms

    Hybrid Public Key:
    {session["hybrid_public_key"]} Bytes

    ML-KEM Public Key:
    {session["mlkem_public_key"]} Bytes

    X25519 Public Key:
    {session["x25519_public_key"]} Bytes
    """

        QMessageBox.information(
            self,
            "Secure Connection Report",
            report
        )
...
if __name__ == "__main__":
    ...
    # Correct flag — ignores ALL cert errors (development only)
    os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = (
        "--proxy-server=http://127.0.0.1:8443 "
        "--ignore-certificate-errors"
    )

    # Point SSL to your proxy CA certificate
    ...
    app.setWindowIcon(
        QIcon(os.path.join('icons', 'browser_icon.png'))
    )
